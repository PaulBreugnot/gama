/*******************************************************************************************************
 *
 * Utils.java, in gama.ext.traffic, is part of the source code of the GAMA modeling and simulation platform (v.2.0.0).
 *
...
public class Utils {
	...
	public static Triple<IAgent, Double, Boolean> findLeader(final IScope scope, final IAgent vehicle,
		...
		if (road != null && lowestLane <= RoadSkill.getNumLanesTotal(road) - numLanesOccupied) {
			Triple<IAgent, Double, Boolean> triple = findNeighborOnCurrentRoad(scope, vehicle, lowestLane, true);
		...
		if (nextRoad == null || !readyToCross(scope, vehicle, target, nextRoad))
			...
			// NOTE: the added minSafetyDist is necessary for the vehicle to ignore the safety dist when stopping at an
			// endpoint
		...
		IAgent nextTarget = !willViolateOneway ? RoadSkill.getTargetNode(nextRoad) : RoadSkill.getSourceNode(nextRoad);
		...
		for (int i = 0; i < numLanesOccupied; i += 1) {
			...
			if (tmpLeader == null || tmpLeader.dead()) { continue; }
			boolean tmpSameDirection = nextTarget == getCurrentTarget(tmpLeader);
			double extraGap = !wrongDirection ? RoadSkill.getTotalLength(nextRoad) - tmpLeaderDist : tmpLeaderDist;
			double otherVL = getVehicleLength(tmpLeader);
			double gap = distToCurrentTarget + extraGap - 0.5 * vL - 0.5 * otherVL;

			if (gap < 0) return ImmutableTriple.of(tmpLeader, gap, tmpSameDirection);
			if (gap < minGap) {
				minGap = gap;
				leader = tmpLeader;
		...
		if (leader != null) return ImmutableTriple.of(leader, minGap, sameDirection);
	...
	public static Triple<IAgent, Double, Boolean> findFollower(final IScope scope, final IAgent vehicle,
		...
		double minGap = Double.MAX_VALUE;
		boolean sameDirection = false;
		IAgent sourceNode = target == RoadSkill.getTargetNode(road) ? RoadSkill.getSourceNode(road) : target;
		for (IAgent prevRoad : RoadNodeSkill.getRoadsIn(sourceNode)) {
			...
			for (int i = 0; i < numLanesOccupied; i += 1) {
				...
				if (lane >= numLanesTotal - numLanesOccupied) { break; }

				boolean isLinkedLane = lane >= numLanes;
				if (isLinkedLane) { prevRoad = RoadSkill.getLinkedRoad(prevRoad); }
				OrderedBidiMap<Double, IAgent> distMap =
				...
				double gap = RoadSkill.getTotalLength(road) - distToCurrentTarget + extraGap - 0.5 * vL
						- 0.5 * getVehicleLength(tmpFollower);
				if (gap < 0) return ImmutableTriple.of(tmpFollower, gap, tmpSameDirection);
				if (gap < minGap) {
					minGap = gap;
		...
		if (follower != null && !follower.dead()) return ImmutableTriple.of(follower, minGap, sameDirection);
		return ImmutableTriple.of(null, 1e6, false);
	}
	...
	private static Triple<IAgent, Double, Boolean> findNeighborOnCurrentRoad(final IScope scope, final IAgent vehicle,
			final int lowestLane, final boolean isLeader) {
		IAgent road = getCurrentRoad(vehicle);
		IAgent target = getCurrentTarget(vehicle);
		double distToCurrentTarget = getDistanceToCurrentTarget(vehicle);
		double vL = getVehicleLength(vehicle);
		...
		for (int i = 0; i < numLanesOccupied; i += 1) {
			...
			Double k;
			if (isLeader) {
				k = !wrongDirection ? distMap.nextKey(tmpDistQuery) : distMap.previousKey(tmpDistQuery);
			} else {
				k = !wrongDirection ? distMap.previousKey(tmpDistQuery) : distMap.nextKey(tmpDistQuery);
			}
			// No neighbor on this lane
			if (k == null) { continue; }
			double tmpNeighborDist = k;
			IAgent tmpNeighbor = distMap.get(tmpNeighborDist);
			if (tmpNeighbor == null || tmpNeighbor.dead()) { continue; }
		...
		if (neighbor != null) return ImmutableTriple.of(neighbor, minGap, sameDirection);
		return null;
	}
}
