...
/**
 * An editor area drop adapter to handle transfer types <code>EditorInputTransfer</code>, <code>MarkerTransfer</code>,
...
public class EditorAreaDropAdapter extends DropTargetAdapter {
	 ...
	 */
	public EditorAreaDropAdapter(final IWorkbenchWindow window) {
		this.window = window;
	}
		...
		/* Open Editor for resource */
			...
			for (IResource resource : (IResource[]) event.data) {
				if (resource instanceof IFile) {
					...
					if (!file.isPhantom()) { openNonExternalEditor(page, file); }
				}
			}
		}
	...
	/**
	 * Opens an editor for the given file on the given workbench page in response to a drop on the workbench editor
	 * area. In contrast to other ways of opening an editor, we never open an external editor in this case (since
	 * external editors appear in their own window and not in the editor area). The operation fails silently if there is
	 * no suitable editor to open.
	 *
	 * @param page
	 *            the workbench page
	 * @param file
	 *            the file to open
	 * @return the editor part that was opened, or <code>null</code> if no editor was opened
	 */
	private IEditorPart openNonExternalEditor(final IWorkbenchPage page, final IFile file) {
		IEditorPart result;
		try {
			...
			if (defaultEditorDesc != null && !defaultEditorDesc.isOpenExternal()) {
				...
				if (editorDesc == null) throw new PartInitException(IDEWorkbenchMessages.IDE_noFileEditorFound);
	...
	/**
	 * Opens an editor for the given marker on the given workbench page in response to a drop on the workbench editor
	 * area. In contrast to other ways of opening an editor, we never open an external editor in this case (since
	 * external editors appear in their own window and not in the editor area). The operation fails silently if there is
	 * no suitable editor to open.
	 *
	 ...
	 * @return the editor part that was opened, or <code>null</code> if no editor was opened
	 */
	private IEditorPart openNonExternalEditor(final IWorkbenchPage page, final IMarker marker) {
		IEditorPart result;
		try {
			// get the marker resource file
			if (!(marker.getResource() instanceof IFile)) return null;
			...
			if (result != null) { IDE.gotoMarker(result, marker); }
	...
	/**
	 * Opens an editor for the given editor input and editor id combination on the given workbench page in response to a
	 * drop on the workbench editor area. In contrast to other ways of opening an editor, we never open an external
	 * editor in this case (since external editors appear in their own window and not in the editor area). The operation
	 * fails silently if the editor cannot be opened.
	 *
	 * @param page
	 *            the workbench page
	 * @param editorInput
	 ...
	 * @return the editor part that was opened, or <code>null</code> if no editor was opened
	 */
	private IEditorPart openNonExternalEditor(final IWorkbenchPage page, final IEditorInput editorInput,
