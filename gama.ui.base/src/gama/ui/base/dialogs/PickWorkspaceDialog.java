/*******************************************************************************************************
 *
 * PickWorkspaceDialog.java, in gama.ui.base, is part of the source code of the GAMA modeling and simulation platform
 * (v.2.0.0).
 *
...
import gama.ui.base.shared.ApplicationControl;
...
public class PickWorkspaceDialog extends TitleAreaDialog {
	...
	protected Control createDialogArea(final Composite parent) {
		...
		try {
			...
			final String wsRoot = WorkspaceManager.getLastSetWorkspaceDirectory();
			...
			rememberWorkspaceButton.setSelection(WorkspaceManager.isRememberWorkspace());

			final String lastUsed = WorkspaceManager.getLastUsedWorkspaces();
	 ...
	 * @return Path
	 */
	public String getSelectedWorkspaceLocation() { return WorkspaceManager.getSelectedWorkspaceRootLocation(); }
	...
	protected void okPressed() {
		...
		final String ret =
				WorkspaceManager.checkWorkspaceDirectory(ApplicationControl.getInstance(), str, true, true, cloning);
		if (ret != null) {
		...
		WorkspaceManager.isRememberWorkspace(rememberWorkspaceButton.getSelection());
		WorkspaceManager.setLastUsedWorkspaces(buf.toString());
		...
		WorkspaceManager.setLastSetWorkspaceDirectory(str);
	...
	public static boolean checkAndCreateWorkspaceRoot(final String wsRoot) {
		try {
			...
			File dotFile = new File(wsRoot + File.separator + WorkspaceManager.WORKSPACE_IDENTIFIER);
			if (!dotFile.exists()) {
