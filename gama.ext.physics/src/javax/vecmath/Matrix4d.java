/*******************************************************************************************************
 *
 * Matrix4d.java, in gama.ext.physics, is part of the source code of the
...
public class Matrix4d implements java.io.Serializable, Cloneable {
	...
	/** The Constant EPS. */
	/*
	 * double[] tmp = new double[16]; double[] tmp_rot = new double[9]; // scratch matrix double[] tmp_scale = new
	 * double[3]; // scratch matrix
	 */
	...
	/**
	 * Constructs and initializes a Matrix4d from the quaternion, translation, and scale values; the scale is applied
	 * only to the rotational components of the matrix (upper 3x3) and not to the translational components.
	 *
	 * @param q1
	...
	/**
	 * Constructs and initializes a Matrix4d from the rotation matrix, translation, and scale values; the scale is
	 * applied only to the rotational components of the matrix (upper 3x3) and not to the translational components of
	 * the matrix.
	 *
	 * @param m1
	...
	/**
	 * Constructs and initializes a Matrix4f from the rotation matrix, translation, and scale values; the scale is
	 * applied only to the rotational components of the matrix (upper 3x3) and not to the translational components of
	 * the matrix.
	 *
	 * @param m1
	...
	/**
	 * Performs an SVD normalization of this matrix in order to acquire the normalized rotational component; the values
	...
	/**
	 * Performs an SVD normalization of this matrix in order to acquire the normalized rotational component; the values
	 * are placed into the Matrix3f parameter.
	 *
	 * @param m1
	 *            the matrix into which the rotational component is placed
	 */
	...
	/**
	 * Performs an SVD normalization of q1 matrix in order to acquire the normalized rotational component; the values
	 * are placed into the Quat4d parameter.
	 *
	 * @param q1
	...
	/**
	 * Sets the values in this Matrix4d equal to the row-major array parameter (ie, the first four elements of the array
	 ...
	 * argument; the other elements of this matrix are initialized as if this were an identity matrix (i.e., affine
	...
	/**
	 * Sets the rotational component (upper 3x3) of this matrix to the matrix values in the double precision Matrix3d
	 * argument; the other elements of this matrix are initialized as if this were an identity matrix (i.e., affine
	 * matrix with no translational component).
	 *
	 * @param m1
	...
	/**
	 * Sets the value of this matrix from the rotation expressed by the quaternion q1, the translation t1, and the scale
	 * s.
	 *
	 * @param q1
	...
	final void invertGeneral(final Matrix4d m1) {
		...
		if (!luDecomposition(tmp, row_perm)) // Matrix has no inverse
	 ...
	 * itself. The input parameters are "matrix0" and "dimen". The array "matrix0" is also an output parameter. The
	...
	static void luBacksubstitution(final double[] matrix1, final int[] row_perm, final double[] matrix2) {
		...
		for (k = 0; k < 4; k++) {
			...
			matrix2[cv + 4 * 0] = (matrix2[cv + 4 * 0] - matrix1[rv + 1] * matrix2[cv + 4 * 1]
					- matrix1[rv + 2] * matrix2[cv + 4 * 2] - matrix1[rv + 3] * matrix2[cv + 4 * 3]) / matrix1[rv + 0];
		}
	}
	...
	public final double determinant() {
		...
		det = m00 * (m11 * m22 * m33 + m12 * m23 * m31 + m13 * m21 * m32 - m13 * m22 * m31 - m11 * m23 * m32
				- m12 * m21 * m33);
		det -= m01 * (m10 * m22 * m33 + m12 * m23 * m30 + m13 * m20 * m32 - m13 * m22 * m30 - m10 * m23 * m32
				- m12 * m20 * m33);
		det += m02 * (m10 * m21 * m33 + m11 * m23 * m30 + m13 * m20 * m31 - m13 * m21 * m30 - m10 * m23 * m31
				- m11 * m20 * m33);
		det -= m03 * (m10 * m21 * m32 + m11 * m22 * m30 + m12 * m20 * m31 - m12 * m21 * m30 - m10 * m22 * m31
				- m11 * m20 * m32);
	...
	/**
	 * Sets the value of this transform to a scale and translation matrix; the scale is not applied to the translation
	...
	/**
	 * Modifies the translational components of this matrix to the values of the Vector3d argument; the other values of
	...
	public final void mul(final Matrix4d m1) {
		double m00, m01, m02, m03, m10, m11, m12, m13, m20, m21, m22, m23, m30, m31, m32, m33; // vars for temp result
																								// matrix
	...
	public final void mul(final Matrix4d m1, final Matrix4d m2) {
		if (this != m1 && this != m2) {
			...
			double m00, m01, m02, m03, m10, m11, m12, m13, m20, m21, m22, m23, m30, m31, m32, m33; // vars for temp
																									// result matrix
	...
	/**
	 * Returns true if all of the data members of Matrix4d m1 are equal to the corresponding data members in this
	 * Matrix4d.
	 *
	 * @param m1
	...
	/**
	 * Returns true if the Object t1 is of type Matrix4d and all of the data members of t1 are equal to the
	...
	/**
	 * Returns a hash code value based on the data values in this object. Two different Matrix4d objects with identical
	 * data values (i.e., Matrix4d.equals returns true) will return the same hash code value. Two objects with different
	...
	public final void transform(final Tuple4d vec, final Tuple4d vecOut) {
		...
		vecOut.x = m00 * vec.x + m01 * vec.y + m02 * vec.z + m03 * vec.w;
		vecOut.y = m10 * vec.x + m11 * vec.y + m12 * vec.z + m13 * vec.w;
		vecOut.z = m20 * vec.x + m21 * vec.y + m22 * vec.z + m23 * vec.w;
	}
	...
	public final void transform(final Tuple4d vec) {
		vec.x = m00 * vec.x + m01 * vec.y + m02 * vec.z + m03 * vec.w;
		vec.y = m10 * vec.x + m11 * vec.y + m12 * vec.z + m13 * vec.w;
		vec.z = m20 * vec.x + m21 * vec.y + m22 * vec.z + m23 * vec.w;
	...
	public final void transform(final Tuple4f vec, final Tuple4f vecOut) {
		vecOut.x = (float) (m00 * vec.x + m01 * vec.y + m02 * vec.z + m03 * vec.w);
		vecOut.y = (float) (m10 * vec.x + m11 * vec.y + m12 * vec.z + m13 * vec.w);
		vecOut.z = (float) (m20 * vec.x + m21 * vec.y + m22 * vec.z + m23 * vec.w);
	...
	public final void transform(final Tuple4f vec) {
		vec.x = (float) (m00 * vec.x + m01 * vec.y + m02 * vec.z + m03 * vec.w);
		vec.y = (float) (m10 * vec.x + m11 * vec.y + m12 * vec.z + m13 * vec.w);
		vec.z = (float) (m20 * vec.x + m21 * vec.y + m22 * vec.z + m23 * vec.w);
		vec.w = (float) (m30 * vec.x + m31 * vec.y + m32 * vec.z + m33 * vec.w);
	}

	/**
	 * Transforms the point parameter with this Matrix4d and places the result into pointOut. The fourth element of the
	...
	/**
	 * Transforms the point parameter with this Matrix4d and places the result back into point. The fourth element of
	 * the point input parameter is assumed to be one.
	 *
	 * @param point
	 *            the input point to be transformed.
	 */
	...
	/**
	 * Transforms the normal parameter by this Matrix4d and places the value into normalOut. The fourth element of the
	...
	/**
	 * Transforms the normal parameter by this transform and places the value back into normal. The fourth element of
	 * the normal is assumed to be zero.
	 *
	 * @param normal
	 *            the input normal to be transformed.
	 */
	...
	/**
	 * Sets the rotational component (upper 3x3) of this matrix to the matrix equivalent values of the quaternion
	 * argument; the other elements of this matrix are unchanged; a singular value decomposition is performed on this
	 * object's upper 3x3 matrix to factor out the scale, then this object's upper 3x3 matrix components are replaced by
	 * the matrix equivalent of the quaternion, and then the scale is reapplied to the rotational components.
	 *
	 * @param q1
	...
	/**
	 * Sets the rotational component (upper 3x3) of this matrix to the matrix equivalent values of the axis-angle
	 * argument; the other elements of this matrix are unchanged; a singular value decomposition is performed on this
	 * object's upper 3x3 matrix to factor out the scale, then this object's upper 3x3 matrix components are replaced by
	 * the matrix equivalent of the axis-angle, and then the scale is reapplied to the rotational components.
	 *
	 * @param a1
