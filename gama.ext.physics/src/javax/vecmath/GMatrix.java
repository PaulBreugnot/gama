/*******************************************************************************************************
 *
 * GMatrix.java, in gama.ext.physics, is part of the source code of the
...
/**
 * A double precision, general, dynamically-resizable, two-dimensional matrix class. Row and column numbering begins
...
public class GMatrix implements java.io.Serializable, Cloneable {
	...
	/**
	 * Constructs an nRow by NCol identity matrix. Note that because row and column numbering begins with zero, nRow and
	 * nCol will be one larger than the maximum possible matrix index values.
	 *
	 * @param nRow
	...
	/**
	 * Constructs an nRow by nCol matrix initialized to the values in the matrix array. The array values are copied in
	 * one row at a time in row major fashion. The array should be at least nRow*nCol in length. Note that because row
	 * and column numbering begins with zero, nRow and nCol will be one larger than the maximum possible matrix index
	 * values.
	 *
	 * @param nRow
	 *            number of rows in this matrix.
	...
	public final void mul(final GMatrix m1) {
		...
		if (nCol != m1.nRow || nCol != m1.nCol) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix0"));
	...
	/**
	 * Computes the outer product of the two vectors; multiplies the the first vector by the transpose of the second
	 * vector and places the matrix result into this matrix. This matrix must be be as big or bigger than
	...
	public final void mul(final GVector v1, final GVector v2) {
		...
		if (nRow < v1.getSize()) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix2"));

		if (nCol < v2.getSize()) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix3"));
	...
	public final void add(final GMatrix m1) {
		...
		if (nRow != m1.nRow) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix4"));

		if (nCol != m1.nCol) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix5"));
	...
	public final void add(final GMatrix m1, final GMatrix m2) {
		...
		if (m2.nRow != m1.nRow) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix6"));

		if (m2.nCol != m1.nCol) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix7"));

		if (nCol != m1.nCol || nRow != m1.nRow) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix8"));
	...
	public final void sub(final GMatrix m1) {
		int i, j;
		if (nRow != m1.nRow) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix9"));

		if (nCol != m1.nCol) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix28"));
	...
	public final void sub(final GMatrix m1, final GMatrix m2) {
		int i, j;
		if (m2.nRow != m1.nRow) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix10"));

		if (m2.nCol != m1.nCol) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix11"));

		if (nRow != m1.nRow || nCol != m1.nCol) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix12"));
	...
	public final void negate(final GMatrix m1) {
		int i, j;
		if (nRow != m1.nRow || nCol != m1.nCol) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix13"));
	...
	/**
	 * Copies a sub-matrix derived from this matrix into the target matrix. The upper left of the sub-matrix is located
	 * at (rowSource, colSource); the lower right of the sub-matrix is located at (lastRowSource,lastColSource). The
	 * sub-matrix is copied into the the target matrix starting at (rowDest, colDest).
	 *
	 * @param rowSource
	 ...
	 * @param rowDest
	 *            the top-most row of the position of the copied sub-matrix within the target matrix
	 * @param colDest
	 *            the left-most column of the position of the copied sub-matrix within the target matrix
	 * @param target
	...
	public final void copySubMatrix(final int rowSource, final int colSource, final int numRow, final int numCol,
			final int rowDest, final int colDest, final GMatrix target) {
		int i, j;
	...
	/**
	 * Changes the size of this matrix dynamically. If the size is increased no data values will be lost. If the size is
	 * decreased, only those data values whose matrix positions were eliminated will be lost.
	 *
	 * @param nRow
	...
	/**
	 * Sets the value of this matrix to the values found in the array parameter. The values are copied in one row at a
	 * time, in row major fashion. The array should be at least equal in length to the number of matrix rows times the
	...
	public final void getRow(final int row, final GVector vector) {
		if (vector.getSize() < nCol) { vector.setSize(nCol); }
	...
	public final void getColumn(final int col, final GVector vector) {
		if (vector.getSize() < nRow) { vector.setSize(nRow); }
			...
			if (nCol > 0) {
				if (nRow > 0) {
					...
					if (nRow > 1) {
						...
						if (nRow > 2) { m1.m20 = (float) values[2][0]; }
					}
				}
				if (nCol > 1) {
					if (nRow > 0) {
						...
						if (nRow > 1) {
							...
							if (nRow > 2) { m1.m21 = (float) values[2][1]; }
						}
					}
					if (nCol > 2 && nRow > 0) {
						...
						if (nRow > 1) {
							...
							if (nRow > 2) { m1.m22 = (float) values[2][2]; }
						}
					}
				}
			}
			...
			if (nCol > 0) {
				if (nRow > 0) {
					...
					if (nRow > 1) {
						...
						if (nRow > 2) {
							m1.m20 = (float) values[2][0];
							if (nRow > 3) { m1.m30 = (float) values[3][0]; }
						}
					}
				}
				if (nCol > 1) {
					if (nRow > 0) {
						...
						if (nRow > 1) {
							...
							if (nRow > 2) {
								m1.m21 = (float) values[2][1];
								if (nRow > 3) { m1.m31 = (float) values[3][1]; }
							}
						}
					}
					if (nCol > 2) {
						if (nRow > 0) {
							...
							if (nRow > 1) {
								...
								if (nRow > 2) {
									m1.m22 = (float) values[2][2];
									if (nRow > 3) { m1.m32 = (float) values[3][2]; }
								}
							}
						}
						if (nCol > 3 && nRow > 0) {
							...
							if (nRow > 1) {
								...
								if (nRow > 2) {
									...
									if (nRow > 3) { m1.m33 = (float) values[3][3]; }
								}
							}
						}
					}
	 ...
	 *
	 * @param col
	 *            the column of this matrix into which the array values will be copied
	 * @param array
	 *            the source array
	 */
	...
	public final void transpose(final GMatrix m1) {
		...
		if (nRow != m1.nCol || nCol != m1.nRow) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix17"));
	...
	/**
	 * Returns a hash code value based on the data values in this object. Two different GMatrix objects with identical
	 * data values (i.e., GMatrix.equals returns true) will return the same hash number. Two GMatrix objects with
	 * different data members may return the same hash value, although this is not likely.
	 *
	...
	/**
	 * Returns true if all of the data members of GMatrix m1 are equal to the corresponding data members in this
	 * GMatrix.
	 *
	 * @param m1
	...
	public boolean equals(final GMatrix m1) {
		try {
			...
			if (nRow != m1.nRow || nCol != m1.nCol) return false;

			for (i = 0; i < nRow; i++) {
				for (j = 0; j < nCol; j++) {
					if (values[i][j] != m1.values[i][j]) return false;
				}
			}
			return true;
	...
	/**
	 * Returns true if the Object o1 is of type GMatrix and all of the data members of o1 are equal to the corresponding
	 * data members in this GMatrix.
	 *
	 * @param o1
	...
	public boolean equals(final Object o1) {
		try {
			...
			int i, j;
			if (nRow != m2.nRow || nCol != m2.nCol) return false;

			for (i = 0; i < nRow; i++) {
				for (j = 0; j < nCol; j++) {
					if (values[i][j] != m2.values[i][j]) return false;
				}
			}
			return true;
	 ...
	 * parameter, otherwise returns false. The L-infinite distance is equal to MAX[i=0,1,2, . . .n ; j=0,1,2, . . .n ;
	...
	/**
	 * Finds the singular value decomposition (SVD) of this matrix such that this = U*W*transpose(V); and returns the
	 * rank of this matrix; the values of U,W,V are all overwritten. Note that the matrix V is output as V, and not
	 * transpose(V). If this matrix is mxn, then U is mxm, W is a diagonal matrix that is mxn, and V is nxn. Using the
	 * notation W = diag(w), then the inverse of this matrix is: inverse(this) = V*diag(1/w)*tranpose(U), where
	 * diag(1/w) is the same matrix as W except that the reciprocal of each of the diagonal components is used.
	 *
	 * @param U
	...
	public final int SVD(final GMatrix U, final GMatrix W, final GMatrix V) {
		...
		if (nCol != V.nCol || nCol != V.nRow) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix18"));

		if (nRow != U.nRow || nRow != U.nCol) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix25"));

		if (nRow != W.nRow || nCol != W.nCol) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix26"));
		...
		// 4) slow performance
		if (nRow == 2 && nCol == 2 && values[1][0] == 0.0) {
			U.setIdentity();
			V.setIdentity();

			if (values[0][1] == 0.0) return 2;
	...
	/**
	 * LU Decomposition: this matrix must be a square matrix and the LU GMatrix parameter must be the same size as this
	 * matrix. The matrix LU will be overwritten as the combination of a lower diagonal and upper diagonal matrix
	 * decompostion of this matrix; the diagonal elements of L (unity) are not stored. The GVector parameter records the
	 * row permutation effected by the partial pivoting, and is used as a parameter to the GVector method LUDBackSolve
	 * to solve sets of linear equations. This method returns +/- 1 depending on whether the number of row interchanges
	 * was even or odd, respectively.
	 *
	 * @param LU
	 *            The matrix into which the lower and upper decompositions will be placed.
	 * @param permutation
	...
	public final int LUD(final GMatrix LU, final GVector permutation) {
		...
		if (nRow != nCol) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix19"));

		if (nRow != LU.nRow || nCol != LU.nCol) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix27"));

		if (LU.nRow != permutation.getSize()) throw new MismatchedSizeException(VecMathI18N.getString("GMatrix20"));
		...
		if (!luDecomposition(LU.nRow, temp, row_perm, even_row_exchange)) // Matrix has no inverse
	...
	final void invertGeneral(final GMatrix m1) {
		...
		if (m1.nRow != m1.nCol) // Matrix is either under or over determined
		...
		// Calculate LU decomposition: Is the matrix singular?
		if (!luDecomposition(m1.nRow, temp, row_perm, even_row_exchange)) // Matrix has no inverse
			throw new SingularMatrixException(VecMathI18N.getString("GMatrix21"));
	...
	/**
	 * Given a nxn array "matrix0", this function replaces it with the LU decomposition of a row-wise permutation of
	 * itself. The input parameters are "matrix0" and "dim". The array "matrix0" is also an output parameter. The vector
	 * "row_perm[]" is an output parameter that contains the row permutations resulting from partial pivoting. The
	 * output parameter "even_row_xchg" is 1 when the number of row exchanges is even, or -1 otherwise. Assumes data
	 * type is always double.
	 *
	 * @param dim the dim
	 * @param matrix0 the matrix 0
	 * @param row_perm the row perm
	 * @param even_row_xchg the even row xchg
	...
	static boolean luDecomposition(final int dim, final double[] matrix0, final int[] row_perm,
		...
		for (j = 0; j < dim; j++) {
			...
			if (imax < 0) throw new RuntimeException(VecMathI18N.getString("GMatrix24"));
			...
			// Is the matrix singular
			if (matrix0[mtx + dim * j + j] == 0.0) return false;
	...
	/**
	 * Solves a set of linear equations. The input parameters "matrix1", and "row_perm" come from luDecompostion and do
	 * not change here. The parameter "matrix2" is a set of column vectors assembled into a nxn matrix of floating-point
	 * values. The procedure takes each column of "matrix2" in turn and treats it as the right-hand side of the matrix
	...
	static int computeSVD(final GMatrix mat, final GMatrix U, final GMatrix W, final GMatrix V) {
		...
		if (debug) { System.out.println("input to compute_svd = \n" + m.toString()); }
		...
		for (si = 0; si < sLength; si++) {
			...
			if (nc > 2) {
				...
				if (debug) { System.out.println("mag = " + mag); }
	...
	static int compute_2X2(final double f, final double g, final double h, final double[] single_values,
		...
		if (ga == 0.0) {
			...
			if (pmax == 1) { tsign = d_sign(c_b4, csr[0]) * d_sign(c_b4, csl[0]) * d_sign(c_b4, f); }
			if (pmax == 2) { tsign = d_sign(c_b4, snr[0]) * d_sign(c_b4, csl[0]) * d_sign(c_b4, g); }
			if (pmax == 3) { tsign = d_sign(c_b4, snr[0]) * d_sign(c_b4, snl[0]) * d_sign(c_b4, h); }
