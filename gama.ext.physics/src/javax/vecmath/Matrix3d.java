/*******************************************************************************************************
 *
 * Matrix3d.java, in gama.ext.physics, is part of the source code of the
...
public class Matrix3d implements java.io.Serializable, Cloneable {
	...
	/**
	 * Sets the scale component of the current matrix by factoring out the current scale (by doing an SVD) and
	...
	/**
	 * Sets the values in this Matrix3d equal to the row-major array parameter (ie, the first three elements of the
	...
	/**
	 * Given a 3x3 array "matrix0", this function replaces it with the LU decomposition of a row-wise permutation of
	 * itself. The input parameters are "matrix0" and "dimen". The array "matrix0" is also an output parameter. The
	 * vector "row_perm[3]" is an output parameter that contains the row permutations resulting from partial pivoting.
	...
	/**
	 * Solves a set of linear equations. The input parameters "matrix1", and "row_perm" come from luDecompostionD3x3 and
	 * do not change here. The parameter "matrix2" is a set of column vectors assembled into a 3x3 matrix of
	...
	/**
	 * Multiplies this matrix by matrix m1, does an SVD normalization of the result, and places the result back into
	 * this matrix this = SVDnorm(this*m1).
	 *
	 * @param m1
	...
	/**
	 * Multiplies matrix m1 by matrix m2, does an SVD normalization of the result, and places the result into this
	 * matrix this = SVDnorm(m1*m2).
	 *
	 * @param m1
	...
	/**
	 * Returns true if all of the data members of Matrix3d m1 are equal to the corresponding data members in this
	 * Matrix3d.
	 *
	 * @param m1
	...
	/**
	 * Returns true if the Object t1 is of type Matrix3d and all of the data members of t1 are equal to the
	...
	/**
	 * Returns a hash code value based on the data values in this object. Two different Matrix3d objects with identical
	 * data values (i.e., Matrix3d.equals returns true) will return the same hash code value. Two objects with different
	 ...
	 *
	 * @param m the m
	 * @param outScale the out scale
	 * @param outRot the out rot
	 */
	static void compute_svd(final double[] m, final double[] outScale, final double[] outRot) {
		...
		/*
		 * System.out.println("t1 is \n" + t1); System.out.println("t1="+t1[0]+" "+t1[1]+" "+t1[2]);
		 * System.out.println("t1="+t1[3]+" "+t1[4]+" "+t1[5]); System.out.println("t1="+t1[6]+" "+t1[7]+" "+t1[8]);
		 *
		 * System.out.println("t2 is \n" + t2); System.out.println("t2="+t2[0]+" "+t2[1]+" "+t2[2]);
		 * System.out.println("t2="+t2[3]+" "+t2[4]+" "+t2[5]); System.out.println("t2="+t2[6]+" "+t2[7]+" "+t2[8]);
		 */
	...
	private static final boolean almostEqual(final double a, final double b) {
		...
		if (diff < EPSILON_ABSOLUTE || diff / max < EPSILON_RELATIVE) return true;
