...
public abstract class Tuple3d implements java.io.Serializable, Cloneable {
	...
	/**
	 * Returns a hash code value based on the data values in this object. Two different Tuple3d objects with identical
	 * data values (i.e., Tuple3d.equals returns true) will return the same hash code value. Two objects with different
	...
	/**
	 * Returns true if the Object t1 is of type Tuple3d and all of the data members of t1 are equal to the corresponding
	 * data members in this Tuple3d.
	 *
	 * @param t1
	...
	public boolean epsilonEquals(final Tuple3d t1, final double epsilon) {
		...
		diff = x - t1.x;
		if (Double.isNaN(diff) || ((diff < 0 ? -diff : diff) > epsilon)) return false;
	...
	/**
	 * Linearly interpolates between this tuple and tuple t1 and places the result into this tuple: this =
