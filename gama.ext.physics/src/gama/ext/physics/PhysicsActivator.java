/*******************************************************************************************************
 *
 * PhysicsActivator.java, in gama.ext.physics, is part of the source code of the GAMA modeling and simulation platform
 * (v.2.0.0).
 *
...
public class PhysicsActivator implements BundleActivator {
	...
	public static boolean loadLibrary() {
		if (!NATIVE_BULLET_LIBRARY_LOADED && LOAD_NATIVE_BULLET_LIBRARY) {
			GAMA.initializeAtStartup("native Bullet library", () -> {
				try {
					...
					ERR(">> Impossible to load Bullet native library from " + bundle.getSymbolicName() + " because "
							+ e.getMessage());
