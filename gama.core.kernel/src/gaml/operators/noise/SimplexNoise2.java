/*******************************************************************************************************
 *
 * SimplexNoise2.java, in gama.core.kernel, is part of the source code of the
...
public class SimplexNoise2 {
	...
	/**
	 * Instantiates a new simplex noise 2.
	 *
	 * @param seed the seed
	 */
	public SimplexNoise2(final double seed) {
		p = p_supply.clone();
		// Uses its own random generator, governed by the seed in parameter
		RandomUtils rand = new RandomUtils(seed, "mersenne");
		// the seed determines the swaps that occur between the default order and the order we're actually going to use
		for (int i = 0; i < NUMBEROFSWAPS; i++) {
			int swapFrom = rand.between(0, p.length - 1);
			int swapTo = rand.between(0, p.length - 1);
	 ...
	 */
	public SimplexNoise2(final IScope scope) {
		p = p_supply.clone();
		scope.getRandom().shuffleInPlace(p);
