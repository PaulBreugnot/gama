...
import gama.ext.pedestrian.skills.PedestrianRoadSkill;
...
public class PedestrianNetwork {
	...
	public static IList<IShape> generateNetwork(IScope scope, IList<IContainer<?, ? extends IShape>> obst, 
			...
			boolean cleanNetwork, double toleranceClip, double toleranceTriang, double minDistPath, double simplicationDistance, double sizeSquare){
			DEBUG.ON();
		...
		for (IShape g : lines) {
			if (simplicationDistance > 0) 
				g = Transformations.simplification(scope, g, simplicationDistance);
